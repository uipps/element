...
/*
 进行.git和.git_uipps的目录切换，默认是 change toggle(互相切换)
    1) 如果存在.git_uipps和.git，则将.git ==> .git_ElemeFE_dev , .git_uipps ==> .git
    2) 如果存在.git_ElemeFE_dev和.git，则将.git ==> .git_uipps , .git_ElemeFE_dev ==> .git
    3) 如果同时存在.git_ElemeFE_dev和.git_uipps，则提示错误，需要人工处理。
    4) 如果.git_ElemeFE_dev和.git_uipps两个都不存在，也提示错误，需要人工处理。
...
function main($o) {
    ...
    // 4. 如果.git_ElemeFE_dev和.git_uipps两个都不存在，提示错误，需要人工处理
    ...
    // 3. 如果.git_ElemeFE_dev和.git_uipps两个同时存在，提示错误，需要人工处理
    if (file_exists($l_path . '/' . $l_dir_uipps) && file_exists($l_path . '/' . $l_dir_laravel)) {
